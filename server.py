...
host = "192.168.29.232"
port = 5555  # Choose any randkom port which is not so common (like 80)
...
def ban_user(name):
    if name in nicknames:
        ...
        client_to_kick = clients[name_index]
        clients.remove(client_to_kick)
        client_to_kick.send('You Were Banned from Chat !'.encode('ascii'))
        client_to_kick.close()
        nicknames.remove(name)
        broadcast(f'{name} was banned from the server!'.encode('ascii'))
